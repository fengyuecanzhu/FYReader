...
public abstract class PageLoader {
    ...
    /*****************************init params*******************************/
    PageLoader(PageView pageView, Book book, Callback callback) {
        mPageView = pageView;
        this.book = book;
        ...
        mCurChapterPos = book.getHisttoryChapterNum();
        mCurPagePos = book.getLastReadPosition();
    ...
    public boolean skipNextChapter() {
        if (mCurChapterPos + 1 >= book.getChapterTotalNum()) {
            return false;
        }
    ...
    void pagingEnd(PageAnimation.Direction direction) {
        ...
        switch (direction) {
            case NEXT:
                ...
                } else if (mCurChapterPos < book.getChapterTotalNum() - 1) {
                    mCurChapterPos = mCurChapterPos + 1;
                    mCurPagePos = 0;
                    Collections.swap(chapterContainers, 0, 1);
                    Collections.swap(chapterContainers, 1, 2);
        ...
        book.setHisttoryChapterNum(mCurChapterPos);
        book.setLastReadPosition(mCurPagePos);
    ...
    private synchronized void drawBackground(final Canvas canvas, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (!callback.getChapterList().isEmpty()) {
            String title = callback.getChapterList().size() > txtChapter.getPosition() ? callback.getChapterList().get(txtChapter.getPosition()).getTitle() : "";
            title = ChapterContentHelp.getInstance().replaceContent(book.getName(), book.getTag(), title, book.getReplaceEnable());
            ...
            String progress = (txtChapter.getStatus() != TxtChapter.Status.FINISH) ? ""
                    : getReadProgress(mCurChapterPos, book.getChapterTotalNum(), mCurPagePos, curChapter().txtChapter.getPageSize());
    ...
    private synchronized void drawContent(Bitmap bitmap, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (txtChapter.getStatus() != TxtChapter.Status.FINISH) {
            ...
            for (int i = 0; i < txtPage.getTitleLines(); ++i) {
                ...
                mTitlePaint.setColor(isLight ? mContext.getResources().getColor(R.color.sys_color) : readBookControl.getTextColor());
            ...
            for (int i = txtPage.getTitleLines(); i < txtPage.size(); ++i) {
                ...
                mTextPaint.setColor(isLight ? mContext.getResources().getColor(R.color.sys_color) : readBookControl.getTextColor());
    ...
    void drawContent(final Canvas canvas, float offset) {
        ...
        while (top < totalHeight) {
            ...
            if (pagePos == chapter.getPageSize() - 1) {
                String sign = "\u23af \u23af";
                if (chapterPos == book.getChapterTotalNum() - 1) {
    ...
    boolean hasNext(int pageOnCur) {
        ...
        return mCurChapterPos + 1 < book.getChapterTotalNum();
    }
